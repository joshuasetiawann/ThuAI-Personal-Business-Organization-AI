...
def test_supabase_adapter_blocks_and_audits():
    """Optional Supabase adapter must block under LOCAL_ONLY_MODE and audit the
    attempt when a DB session is supplied. Self-contained (own SQLite)."""
    import asyncio, os
    from sqlalchemy.ext.asyncio import create_async_engine, async_sessionmaker, AsyncSession
    from sqlalchemy import select, func
    from db.base import Base
    from db.models import AuditLog
    from core.local_only import ExternalProviderBlocked
    from adapters.optional.supabase_sync import SupabaseSyncAdapter

    dbfile = "/tmp/tt_supa.db"
    if os.path.exists(dbfile):
        os.remove(dbfile)
    ...
    async def run():
        ...
        async with sm() as db:
            try:
                await SupabaseSyncAdapter().upsert("conversations", {"x": 1}, db=db, actor="tester")
            except ExternalProviderBlocked:
                raised = True
            await db.commit()
            n = (await db.execute(select(func.count()).select_from(AuditLog)
    ...
    raised, n = asyncio.run(run())
    assert raised and n >= 1
