...
export default function Decisions() {
  ...
  const openDetail = (d: DecisionBrief) => {
    setOpen(true); setSel(null); setDErr(null); setDLoading(true); setExecMsg(null); setApprovalId(""); setConfirmingExec(false);
    api.decision(d.id).then((x: any) => setSel(x)).catch((e) => setDErr(e?.message || "Failed")).finally(() => setDLoading(false));
  };
  ...
  return (
    <div className="page">
      ...
      <DetailDrawer open={open} onClose={() => setOpen(false)} title="Decision detail">
        {dLoading ? <Loading /> : dErr ? <ErrorState message={dErr} /> : sel && (
          <div className="kv">
            ...
            <Row k="Agent run" v={sel.agent_run_id ? <code className="mono">{String(sel.agent_run_id).slice(0, 8)}</code> : "—"} />
