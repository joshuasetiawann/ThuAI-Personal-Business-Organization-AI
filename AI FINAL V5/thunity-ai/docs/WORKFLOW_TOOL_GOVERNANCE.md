...
## Pre-Shot 3 hardening notes

- **Allow-listed n8n workflows can still contain external `httpRequest` nodes.** The
  allow-list governs *which* workflow names may run, not their internal nodes. Each
  workflow definition must be **founder-reviewed** before it is added; a local-only
  posture is only as safe as the workflows you register.
- **High/critical tools require a valid, approved `approval_id`.** The executor now
  rejects a missing **or invalid/unapproved** `approval_id` (audited as `tool_blocked`,
  reason `invalid_or_unapproved_approval`). Founder permission alone does **not** bypass
  the gate — an explicit approval record is always required.
- **Never present offline/failed workflow runs as success** in the UI.
