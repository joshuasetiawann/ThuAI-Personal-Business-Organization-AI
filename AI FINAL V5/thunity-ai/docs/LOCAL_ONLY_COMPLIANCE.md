...
**Guarantee (honest hybrid, updated 2026-06-06):** embeddings, the database, the vector
store, automation, and all company data ALWAYS stay on the founder's machine, and there
is **no silent cloud fallback**. Reasoning is local Ollama by default; a SINGLE founder-
declared, key-gated frontier provider (Anthropic or OpenRouter) may handle heavy/strategic
work, always **labelled** per answer. With no frontier key — or under strict
`LOCAL_ONLY_MODE` — no external AI path is reachable at all (every attempt is blocked and
audited).

## How it is enforced

**Single declared frontier surface only.** `config.py` exposes config ONLY for the one
declared frontier lane (`ANTHROPIC_API_KEY` / `OPENROUTER_API_KEY` + model + `FRONTIER_ENABLED`),
gated by `assert_frontier_allowed()`. There is **no** configuration surface for any OTHER
cloud AI / vector provider (OpenAI, Gemini, Groq, Pinecone, Weaviate, …). A separate
quarantined legacy adapter (see below) remains disabled.
...
- `compliance_status()` → `compliant` (fully local, no frontier) | `hybrid` (local-first +
  one declared, labelled, key-gated frontier provider) | `warning` / `error` (misconfigured).
