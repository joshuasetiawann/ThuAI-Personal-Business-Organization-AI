...
class User(Base, TimestampMixin):
    ...
    # Bumped on logout / "sign out everywhere" so previously-issued JWTs (which carry
    # the value as the `tv` claim) are rejected — stateless token revocation.
...
# ── Long-term memory (the hybrid edge: frontier answers grounded in YOUR facts) ──
class Memory(Base, TimestampMixin):
    """A durable, reusable fact about the founder / company / projects.

    Curated locally by the Memory Agent and injected into BOTH local and frontier
    prompts so even a frontier model answers with the founder's private context.
    This is a private knowledge store — it never touches the governance ledgers
    (decisions / tasks / approvals / workflows), so Fast Chat stays side-effect-free
    ...
    kind: Mapped[str] = mapped_column(String(40), default="fact")  # fact|preference|project|person|goal|decision|constraint|misc
    content: Mapped[str] = mapped_column(Text)
    importance: Mapped[int] = mapped_column(Integer, default=3)    # 1..5
    source: Mapped[str] = mapped_column(String(60), default="memory_agent")
...
class Document(Base, TimestampMixin):
    ...
    conversation_id: Mapped[Optional[uuid.UUID]] = mapped_column(GUID, index=True)  # set when this doc is an ingested conversation transcript
...
class AppSetting(Base):
    """Runtime-tunable app settings (founder-controlled in the UI), persisted so they
    survive restarts. Distinct from config.py (env/boot config). e.g.
    auto_ingest_conversations."""
