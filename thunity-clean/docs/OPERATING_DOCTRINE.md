...
**1. Local-only by default.** The entire reasoning core runs on local Ollama models.
There is no OpenAI / Anthropic / Gemini / Groq / Supabase / Pinecone / Weaviate path
in the core. `LOCAL_ONLY_MODE` is `true` by default and hard-blocks external providers
(`core/local_only.py`). There is no silent cloud fallback anywhere — a blocked call
raises `ExternalProviderBlocked` and is audited. See `LOCAL_ONLY_COMPLIANCE.md`.
