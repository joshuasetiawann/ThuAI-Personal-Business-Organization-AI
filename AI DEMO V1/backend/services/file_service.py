...
(re-export from file_manager module)
"""
...
class FileManagerService:
    ...
    async def read_file(self, path: str) -> Dict:
        ...
        if ext not in self.allowed_extensions:
            raise PermissionError(f"Extension {ext} tidak diizinkan")
        if ext in {'.png', '.jpg', '.jpeg', '.gif'}:
            return {"type": "binary", "path": path, "message": "File gambar"}
        async with aiofiles.open(full_path, 'r', encoding='utf-8', errors='ignore') as f:
            content = await f.read()
        return {"path": path, "content": content, "size": full_path.stat().st_size, "extension": ext}
    ...
    async def write_file(self, path: str, content: str, create_dirs: bool = True) -> Dict:
        ...
        return {"success": True, "path": path, "size": full_path.stat().st_size, "message": f"File berhasil ditulis: {path}"}
            ...
            if file_path.is_file() and file_path.suffix in {'.txt', '.md', '.json', '.csv', '.py', '.js'}:
                try:
                    ...
                    if query.lower() in content.lower():
                        lines = [(i+1, line) for i, line in enumerate(content.splitlines()) if query.lower() in line.lower()]
                        results.append({
