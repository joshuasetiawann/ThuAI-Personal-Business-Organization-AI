#!/bin/bash
# Tulis HTML baru LANGSUNG ke dalam container
# Tidak butuh file di disk lokal
# Jalankan dari mana saja: bash inject-chatbot.sh

echo "▶ Menulis HTML baru langsung ke container ai-chatbot..."

sudo docker exec ai-chatbot sh -c 'cat > /usr/share/nginx/html/index.html' << 'HTMLEOF'
<!DOCTYPE html>
<html lang="id">
<head>
<meta charset="UTF-8">
...
:root{--bg:#0c0e13;--bg2:#12151f;--bg3:#1a1d2e;--bg4:#1e2235;--border:#252a3d;--border2:#2d3348;--analyst:#4f8ef7;--critic:#f7614f;--synth:#4fca8e;--accent:#a78bfa;--text:#dde3f0;--text2:#7a84a0}
...
.mb{padding:5px 14px;border-radius:20px;border:1px solid var(--border2);background:transparent;color:var(--text2);font-size:12px;cursor:pointer;transition:all .2s}
...
.ci{padding:9px 12px;border-radius:8px;cursor:pointer;margin-bottom:2px;border-left:2px solid transparent}
...
.sb2{width:40px;height:40px;border-radius:10px;border:none;background:var(--accent);color:#fff;cursor:pointer;font-size:15px;flex-shrink:0;transition:opacity .2s}
...
<div id="scprev" style="display:none;position:fixed;bottom:90px;right:16px;width:210px;border-radius:10px;overflow:hidden;border:2px solid var(--accent);background:var(--bg2);z-index:100">
  <div style="padding:5px 10px;background:var(--accent);color:#fff;font-size:10px;font-weight:700;font-family:monospace;display:flex;justify-content:space-between">
    🖥️ SCREEN <button onclick="stopScreen()" style="background:none;border:none;color:white;cursor:pointer;font-size:11px">✕</button>
  </div>
...
let mode='collaborative',hist=[],media=[],busy=false,tts=false,rec=null,scStream=null;
...
async function send(){
  ...
  addUser(msg,media);
  media=[];document.getElementById('mprev').innerHTML='';
...
function sysmsg(txt){
  const msgs=document.getElementById('msgs');
  const div=document.createElement('div');
  div.className='b sys ani';div.textContent=txt;msgs.appendChild(div);scrl();
}
...
function updMprev(){
  document.getElementById('mprev').innerHTML=media.map((m,i)=>{
    ...
    return'<div style="display:flex;align-items:center;gap:5px;padding:4px 8px;background:var(--bg3);border-radius:6px;font-size:11px">📄 '+m.name+'<button onclick="rmMedia('+i+')" style="background:none;border:none;color:var(--text2);cursor:pointer">x</button></div>';
  }).join('');
}
...
echo ""
echo "✅ SELESAI!"
echo "Buka http://localhost:3001 lalu tekan Ctrl+Shift+R"
