...
set -euo pipefail
if [[ "${1:-}" == "--help" || "${1:-}" == "-h" ]]; then
  echo "Usage: restore-local.sh <archive.tgz|archive.tgz.gpg>"
  echo "Restores Postgres dump + local data/ from a backup archive. Local-only."
  echo "The real .env is never part of a backup — recreate it from .env.example."
  exit 0
fi
