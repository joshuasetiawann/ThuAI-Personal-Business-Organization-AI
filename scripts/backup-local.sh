...
set -euo pipefail
if [[ "${1:-}" == "--help" || "${1:-}" == "-h" ]]; then
  echo "Usage: backup-local.sh        (env: BACKUP_DIR, BACKUP_GPG_RECIPIENT)"
  echo "Local-only, timestamped backup: Postgres dump + data/ + n8n-workflows/ + .env.example."
  echo "NEVER backs up the real .env or any secret; never uploads anywhere."
  exit 0
fi
...
ARCHIVE="$OUT/thunity_backup_${TS}_$$.tgz"
