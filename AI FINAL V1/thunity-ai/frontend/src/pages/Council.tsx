...
export default function Council() {
  ...
  return (
    <div className="page">
      ...
      <Card title="New session" hint={mode === "fast" ? "POST /api/agents/chat" : "POST /api/agents/council"}>
        ...
        {mode === "fast" ? (
          <div className="fast-mode-head">
            <div>
              <div className="fast-mode-title">Fast Chat</div>
              <div className="fast-mode-subtitle">One local model for quick operational answers.</div>
            </div>
          ...
          <div className="muted small" style={{ marginBottom: "10px" }}>AI Council — multi-agent deliberation for important decisions (slower, ~2–4 min).</div>
        )}
        {mode === "council" && (
          <>
        ...
        <textarea ref={promptRef} className="council-input" rows={mode === "fast" ? 3 : 5} value={prompt} disabled={busy || fastBusy}
      ...
      {mode === "fast" && fastTurns.length > 0 && (
        <Card title="Fast Assistant"
          actions={fastConvId ? <a className="link" href={`/conversations?open=${encodeURIComponent(fastConvId)}`}>Open conversation →</a> : undefined}>
          <div className="fast-thread">
            {fastTurns.map((t) => (
              <div key={t.id} className="fast-turn">
                ...
                <div className="fast-assistant">
                  ...
                  {t.status === "pending"
                    ? <div className="muted" style={{ marginTop: "4px" }}><span className="spinner" /> Thinking locally…</div>
                    : t.status === "error"
                      ? <div className="bad small" style={{ marginTop: "4px" }}>⚠ {t.error || "Fast chat failed."}</div>
                      : <>
