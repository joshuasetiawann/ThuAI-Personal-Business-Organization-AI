        ...
        {dLoading ? <Loading /> : dErr ? <ErrorState message={dErr} /> : sel && (
          <div className="kv">
            ...
            <Row k="From decision" v={sel.source_decision_id
              ? <a className="link" href={`/decisions?open=${encodeURIComponent(String(sel.source_decision_id))}`}><code className="mono">{String(sel.source_decision_id).slice(0, 8)}</code></a>
              : "—"} />
